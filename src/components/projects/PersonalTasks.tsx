          ...
          <div className="personal-task-list">{tasks.map((task) => (
            <article key={task._id} className={`personal-task-card task-${task.status}`}>
              ...
              <div className="personal-task-actions">
                ...
                {(task.isMine || task.isReviewer) && !["completed", "verified"].includes(task.status) ? <button className="secondary-button" type="button" onClick={() => setOpenTaskId((current) => current === task._id ? null : task._id)}>{task.isReviewer && ["submitted", "review"].includes(task.status) ? "Review Task" : "Evidence & Review"}</button> : null}
