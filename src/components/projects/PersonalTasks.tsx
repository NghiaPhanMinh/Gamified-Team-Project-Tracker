          ...
          <div className="personal-task-list">{tasks.map((task) => (
            <article key={task._id} className={`personal-task-card task-${task.status}`}>
              ...
              <div className="personal-task-actions">
                ...
                {task.isOpenForClaiming ? <button className="primary-button" type="button" disabled={pendingTaskId === task._id} onClick={() => void run(task._id, () => claimTask({ taskId: task._id }))}>MayLamDi</button> : null}
                {task.isMine && task.status === "todo" && task.acceptanceStatus !== "pending" ? <button className="primary-button" type="button" disabled={pendingTaskId === task._id} onClick={() => void run(task._id, () => updateStatus({ taskId: task._id, status: "in_progress" }))}>MayLamDi</button> : null}
