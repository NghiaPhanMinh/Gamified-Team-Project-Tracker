...
export function normalizeTeamName(value: string) {
  ...
  if (name.length < 2) {
    throw new ConvexError("Team name must contain at least 2 characters.");
  }

  if (name.length > 60) {
    throw new ConvexError("Team name must contain no more than 60 characters.");
  }
...
export function assertValidJoinCode(value: string) {
  ...
  if (
    ...
    throw new ConvexError("Enter a valid 6-character team code.");
  }
...
export function normalizeSharedNote(value: string) {
  ...
  if (note.length < 1) {
    throw new ConvexError("Write a short update before sharing it.");
  }

  if (note.length > 280) {
    throw new ConvexError("The shared update must be 280 characters or fewer.");
  }
